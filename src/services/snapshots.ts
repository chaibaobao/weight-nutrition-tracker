...
import { calculate15DayAverageWeight, getAffectedSnapshotDates } from '../utils/weight';

export async function recalculateSnapshot(date: string): Promise<void> {
  const [profile, weights, meals, existingSnapshot] = await Promise.all([db.profiles.get('profile'), db.weights.toArray(), db.meals.where('date').equals(date).toArray(), db.snapshots.get(date)]);
  if (!profile) return;
  ...
  await db.snapshots.put({
    ...
    algorithmVersion: existingSnapshot?.algorithmVersion ?? ALGORITHM_VERSION, updatedAt: Date.now(),
  });
}
...
export async function recalculateAffectedSnapshots(changedDate: string): Promise<void> {
  ...
  const dates = getAffectedSnapshotDates(changedDate, [...weights.map(w => w.date), ...meals.map(m => m.date), ...snapshots.map(s => s.date)]);
  for (const date of dates) await recalculateSnapshot(date);
}
