...
function HistoryDetail({ date, data, snapshot, onClose, reload, toast }: { date: string; data: AppData; snapshot?: DailySnapshot; onClose: () => void; reload: () => Promise<void>; toast: (text: string) => void }) {
  ...
  return <Modal title="历史详情" onClose={onClose} wide><div className="history-detail"><p className="eyebrow">{formatFriendlyDate(date,true)}</p><div className="detail-weight"><div><span>体重</span><strong>{weight ? `${formatWeight(displayWeight(weight.weightKg))} ${unitLabel}` : '未记录'}</strong></div><button onClick={() => setWeightEdit(true)}>{weight ? '编辑' : '添加体重'}</button></div>
    {snapshot ? <><div className="detail-highlights"><div><span>15日平均</span><strong>{formatWeight(displayWeight(snapshot.average15DayKg))} {unitLabel}</strong></div><div><span>基础代谢参考</span><strong>{roundKcal(snapshot.bmr)} kcal</strong></div><div><span>当日方案</span><strong>{PLAN_LABEL[snapshot.plan]}</strong></div></div><p className="tiny-note">基础代谢仅作为参考，不参与每日营养目标计算。算法版本：{snapshot.algorithmVersion}</p><div className="detail-nutrition"><CompareRow label="热量" actual={actual.calories} target={snapshot.targetCalories} unit="kcal" /><CompareRow label="蛋白质" actual={actual.protein} target={snapshot.proteinTarget} unit="g" /><CompareRow label="碳水" actual={actual.carbs} target={snapshot.carbsTarget} unit="g" /><CompareRow label="脂肪" actual={actual.fat} target={snapshot.fatTarget} unit="g" /></div></> : <p className="inline-empty">当天没有足够体重数据生成营养目标。</p>}
