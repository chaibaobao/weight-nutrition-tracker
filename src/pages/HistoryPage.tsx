...
export function HistoryPage({ data, reload, toast }: { data: AppData; reload: () => Promise<void>; toast: (text: string) => void }) {
  ...
  return <div className="history-page page-enter"><header className="page-header"><div><p>让变化慢慢变得清晰</p><h1>历史</h1></div><span className="header-art"><TrendingUp size={24} /></span></header>
    ...
    {allDates.length === 0 ? <EmptyState title="变化正在路上" text="记录几天后，这里就会出现你的变化趋势。" /> : <>
      ...
      <section className="daily-history"><div className="section-heading"><div><CalendarDays size={20} /><h2>每日记录</h2></div></div><div className="day-list">{allDates.slice().reverse().map(date => { const weight = data.weights.find(w => w.date === date); const average = calculate15DayAverageWeight(data.weights, date); const weightPair = weight ? getWeightDisplayPair(weight.weightKg, preferredUnit) : null; const averagePair = average === null ? null : getWeightDisplayPair(average, preferredUnit); return <button className="day-row" key={date} onClick={() => setSelectedDate(date)}><div className="day-row-copy"><span className="day-date">{formatFriendlyDate(date)}</span>{weightPair ? <div className="day-weight-pair"><strong>{roundSmart(weightPair.primary.value,2)} {weightPair.primary.unit}</strong><span>≈ {roundSmart(weightPair.secondary.value,2)} {weightPair.secondary.unit}</span></div> : <strong className="day-weight-missing">未记录体重</strong>}{averagePair ? <div className="day-average-pair"><span>15日平均 {roundSmart(averagePair.primary.value,2)} {averagePair.primary.unit}</span><small>≈ {roundSmart(averagePair.secondary.value,2)} {averagePair.secondary.unit}</small></div> : <span className="day-average">暂无平均数据</span>}</div><ChevronRight className="day-chevron" size={20} /></button>; })}</div></section>
    </>}
